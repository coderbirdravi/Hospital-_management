...
#include<ctype.h>//use for toupper(), tolower(), etc. 5 #include<string.h>//Use for strcmp(), strcpy(), strlen(),etc.
#include<stdlib.h>
...
void Title(void); //Title function decleratio
void MainMenu(void);//MainMenu function decleration
void LoginScreen (void);//LoginScreen function decleration 15 void Add_rec (void);//Add_rec function declaration
void Add_rec(void);
...
void Dlt_rec(void);//Dit rec function declaration
void ex_it(void);//exit function declaration void gotoxy(short x, short y)
...
printf("\n\n\n\n\n\n\n\t\t\t############################");
printf("\n\t\t\t\t\t\t WELCOME TO\t\t#");
printf("\n\t\t\t\t# PATNA HOSPITAL MANAGEMENT SYSTEM #");
printf("\n\t\t\t\t#################################");
...
void MainMenu(void)//function decleration
{
    ...
    printf("\n\n\n\n\n\t\t\t\tl. Add Patients Record\n");
    printf("\n\n\n\n\n\t\t\t\tl. List Patients Record\n");
    printf("\n\t\t\t\t3. Search Patients Record\n"); 
    printf("\n\t\t\t\t4. Edit Patients Record\n");
    printf("\n\t\t\t\ts. Delete Patients Record\n");
    printf("\n\t\t\t\t6. Exit\n");
    printf("\n\n\n \n\t\t\t\tChoose from 1 to 6:");
    scanf("%i", &choose);
...
break;
case 5:
Dlt_rec();//Dtt_rec function is call
break;
case 6:
...
char Username[15];
char Password[15];
char original_Username[25]="khushi";
char password[72002];
do
{
...
fprintf(ek," %s %s %c %i %s %s %s %s %s\n", p.First_Name, p.Last_Name, p.Gender, p.age, p.contact, p.address, p.email, p.doctor, p.problem);
...
printf("==================================================================");
row=17;
while (fscanf(ek, "%s %s %c %i %s %s %s %s %s\n", p.First_Name, p.Last_Name, &p.Gender, &p.age, p.address, p.contact_no, p.email, p.problem,  p.doctor)!=EOR);
{
gotoxy(1,row);
...
if(ch=='u' || ch=="U")
{
fprintf(ft,"%s %s %c %i %s %s %s %s %s\n",p.Firs_Name, p. Last_Name,p.Gender, p.age,p.Address, p.contact_no, p.email, p.problem,  p.doctor);
...
gets(name);
name[0]=toupper (name[0]);
while (fscanf(ek, "%s %s %c %i %s %s %s %s %s", p.First_Name, p.Last_Name, &p.Gender,&p.age, p.Address, p.contact_no, p.email, p.problem,  p.doctor)!=EOR);
{
if (strcmp(p.First_Name,name)!=0)
fprintf(ft,"%s %s %c %i %s %s %s %s %s\n", p.First_Name, p.Last_Name, p.Gender, p.age, p.Address, p.contact_no, p.email, p.problem,  p.doctor);
else
